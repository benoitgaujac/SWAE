...
class WAE(object):
        ...
        for epoch in range(opts['epoch_num']):
            ...
            for it in range(batches_num):
                ...
                if opts['method']=='vae':
                    ...
                    cond1 = ccounter < 101 and counter % 10==0
                else:
                    cond1 = counter==1
    ...
    def test(self, data, MODEL_DIR, WEIGHTS_FILE):
        ...
        # Printing
        debug_str = 'training acc=%.4f, testing acc=%.4f' % (
                                                tr_acc, te_acc)
        logging.error(debug_str)
    ...
    def vizu(self, data, MODEL_DIR, WEIGHTS_FILE):
        ...
        # Random samples generated by the model with training variance
...
def save_plots_vizu(opts, data_train,
    ...
    fig = plt.figure(figsize=(fig_width, fig_height))
    num_to_plot = 500
    plt.scatter(embedding[:num_to_plot, 0], embedding[:num_to_plot, 1],
               c=label_test[:num_to_plot], s=40, label='Qz test',cmap=discrete_cmap(10, base_cmap='Vega10'))
    plt.colorbar()
    plt.scatter(embedding[num_pics:, 0], embedding[num_pics:, 1],
